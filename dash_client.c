...
static u32 find_max_reward(GF_DASH_Group *group, u32 max_quality, u32 n, u32 i, u32 arr[], Double bandwidth, u32 *new_index, Double max_reward) {
    ...
    if (i==n) {
        Double cur_rebuffer_time = 0.0;
        Double cur_buffer = group->buffer_occupancy_ms; //Millisecond
        Double bitrate_sum = 0.0;
        Double smoothness_diffs = 0.0;
        u32 last_quality = group->active_rep_index;
        for (int j=0; j<n; j++) {
            u32 chunk_quality = arr[j];
            Double chunk_size = group->total_size * (Double)(chunk_quality+1) / (Double)(group->active_rep_index+1);
            // Double download_time = chunk_size/bandwidth;
            Double download_time = (Double)group->last_segment_time - (Double)group->download_start_time;
            download_time *=  ((Double)(chunk_quality+1) / (Double)(group->active_rep_index+1));
            if (cur_buffer < download_time) {
                cur_rebuffer_time += (download_time - cur_buffer);
                cur_buffer = 0.0;
            }
            else {
                cur_buffer -= download_time;
            }
            //TODO: cur_buffer += 4
            cur_buffer += (Double)group->segment_duration;
            Double chunk_bandwidth = (Double)((GF_MPD_Representation *)gf_list_get(group->adaptation_set->representations, chunk_quality))->bandwidth;
            // bitrate_sum += chunk_bandwidth;
            Double last_chunk_bandwidth = (Double)((GF_MPD_Representation *)gf_list_get(group->adaptation_set->representations, last_quality))->bandwidth;
            // smoothness_diffs += abs(chunk_bandwidth - last_chunk_bandwidth);
    ...
    for (int j=0; j<max_quality; j++) {
        u32 temp = arr[i];
        arr[i] = j;
        max_reward = find_max_reward(group, max_quality, n, i+1, arr, bandwidth, new_index, max_reward);
        arr[i] = temp;
    }
    return max_reward;
}
                                              ...
                                              GF_MPD_Representation *rep, Bool go_up_bitrate)
{
    ...
    {
        group->past_bandwidth = gf_list_new();
    }

    while (gf_list_count(group->past_bandwidth) > 0 && *((Double *)gf_list_get(group->past_bandwidth, 0)) == 0.0) {
    ...
    Double *throughputMeasured = (Double *)malloc(sizeof(Double));
    *throughputMeasured = ((group->active_bitrate * group->segment_duration) / (group->last_segment_time - group->download_start_time));

    if (gf_list_count(group->past_bandwidth) == 5)
        gf_list_rem(group->past_bandwidth, 0);

    gf_list_add(group->past_bandwidth, throughputMeasured);

    for (int i=0; i<count; i++) {
        bandwidth_sum += (1.0000/ *((Double *)gf_list_get(group->past_bandwidth,i)));
    }

    Double future_bandwidth = 1.0/(bandwidth_sum/(Double)count);
    Double max_reward = -100000.0;
    ...
    u32 max_quality = gf_list_count(group->adaptation_set->representations);
    max_reward = find_max_reward(group, max_quality, count, 0, arr, future_bandwidth, &new_index, max_reward);
